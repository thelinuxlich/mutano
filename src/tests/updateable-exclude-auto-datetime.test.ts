...
`
    ...
    // ✅ Insertable schema should include auto-generated datetime fields with .optional()
    expect(postContent).toContain('insertable_post')
    expect(postContent).toMatch(/insertable_post[^}]+createdAt:\s*z\.date\(\)\.optional\(\)/)
    expect(postContent).toMatch(/insertable_post[^}]+updatedAt:\s*z\.date\(\)\.optional\(\)/)
